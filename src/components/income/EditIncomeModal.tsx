...
import { updateIncome, getCategories, getClients, supabase } from '../../services/api';
...
const CURRENCY_NAMES: Record<string, string> = {
  ...
  // Load initial values when income changes
  useEffect(() => {
    if (income) {
      setAmount(income.amount.toString());
      setDescription(income.description);
      setSelectedCategory(income.category_id || '');
      setSelectedClient(income.client_id || '');
      setSelectedDate(parseISO(income.date));
      setReferenceNumber(income.reference_number || '');
      setCurrency(income.currency || baseCurrency);
      if (income.tax_rate && income.tax_rate > 0) {
        setTaxRate(income.tax_rate.toString());
        setIncludeTax(true);
      } else {
        setTaxRate("0");
        setIncludeTax(false);
  ...
  try {
    ...
    await updateMutation.mutateAsync({ id: income.id, updates });
  } catch (error) {
    console.error('Update error:', error);
  } finally {
    setLoading(false);
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    ...
                    {clients.map((client: any) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.selectionChip,
                          selectedClient === client.id && styles.selectionChipSelected,
                        ]}
                      ...
                      >
                        <Text
                          style={[
                            styles.selectionChipText,
                            selectedClient === client.id && styles.selectionChipTextSelected,
                          ]}
                        >
                          {client.name}
                        </Text>
